                ...
                <div class="cd-full-width">
                    <div class="container-fluid js-tm-page-content" data-page-no="1" data-page-type="gallery">
                        <div class="tm-img-gallery-container">
                            ...
                            $dsn = 'mysql:dbname=medic; port=3308; host=localhost';
                            $user = 'root';
                            $password = '';
                            try {
                           ...
                           /* <strong>CodeCIS</strong> : <?php echo $donnees['CodeCIS']; ?><br />
                            Dénomination du médicament : <?php echo $donnees['DenominationMedicament']; ?>*/
                            
                            $articles = $bdd->query('SELECT DenominationMedicament,FormePharmaceutique,VoiesAdministration FROM cis_bdpm ORDER BY DenominationMedicament DESC');
                            if (isset($_GET['q']) and !empty($_GET['q'])) {
                                $q = htmlspecialchars($_GET['q']);
                                $articles = $bdd->query('SELECT DenominationMedicament,FormePharmaceutique,VoiesAdministration FROM cis_bdpm WHERE DenominationMedicament LIKE "%' . $q . '%" ORDER BY DenominationMedicament DESC');
                                if ($articles->rowCount() == 0) {
                                    $articles = $bdd->query('SELECT DenominationMedicament,FormePharmaceutique,VoiesAdministration FROM cis_bdpm WHERE CONCAT(DenominationMedicament) LIKE "%' . $q . '%" ORDER BY DenominationMedicament DESC');
                                }
                            }
                            ?>
                            <form method="GET">
                            ...
                            <?php if ($articles->rowCount() > 0) { ?>
                                <p>
                                    <?php while ($a = $articles->fetch()) { ?>
                                       <?= $a['DenominationMedicament'], ' ',$a['FormePharmaceutique'] ,' ',$a['VoiesAdministration']?> <br> 
